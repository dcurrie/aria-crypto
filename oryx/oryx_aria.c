...
#define ERROR_INVALID_KEY_LENGTH -1
#define NO_ERROR 0
...
aria_encrypt_256 (aria_u128_t KeyLeft, aria_u128_t KeyRight, aria_u128_t Plaintext)
{
  ...
  AriaContext ac;
  union { uint8_t Cb[16]; aria_u128_t C; } u;

  ariaInit(&ac, (const uint8_t *)&eh, 32);

  ariaEncryptBlock(&ac, (const uint8_t *)&Plaintext, u.Cb);
  return u.C;
}

aria_u128_t 
aria_decrypt_256 (aria_u128_t KeyLeft, aria_u128_t KeyRight, aria_u128_t Plaintext)
{
  struct eh_s { aria_u128_t kh; aria_u128_t kl; } eh = { KeyLeft, KeyRight };
  AriaContext ac;
  union { uint8_t Cb[16]; aria_u128_t C; } u;
  ...
  ariaDecryptBlock(&ac, (const uint8_t *)&Plaintext, u.Cb);
  return u.C;
}
